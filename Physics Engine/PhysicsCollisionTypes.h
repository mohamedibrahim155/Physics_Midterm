...
static cAABB CalculateAABB(const std::vector<Vertex>& vertices)
{
	...
	glm::vec3 min = vertices[0].vPos;
	glm::vec3 max = vertices[0].vPos;

	for (const Vertex& vertex : vertices)
	{
		min.x = min(min.x, vertex.vPos.x);
		min.y = min(min.y, vertex.vPos.y);
		min.z = min(min.z, vertex.vPos.z);

		max.x = max(max.x, vertex.vPos.x);
		max.y = max(max.y, vertex.vPos.y);
		max.z = max(max.z, vertex.vPos.z);
	}
	...
	std::vector<glm::vec3>& collisionNormal)
{
	...
	intersectionAABB.minV.x = max(a.minV.x, b.minV.x);
	intersectionAABB.minV.y = max(a.minV.y, b.minV.y);
	intersectionAABB.minV.z = max(a.minV.z, b.minV.z);

	intersectionAABB.maxV.x = min(a.maxV.x, b.maxV.x);
	intersectionAABB.maxV.y = min(a.maxV.y, b.maxV.y);
	intersectionAABB.maxV.z = min(a.maxV.z, b.maxV.z);
...
static bool SphereTriangleCollision(const cSphere& sphere, const Triangle& triangle)
{
	...
	float minProj = min(triangleProj[0], min(triangleProj[1], triangleProj[2]));
	float maxProj = max(triangleProj[0], max(triangleProj[1], triangleProj[2]));
	...
	std::vector<glm::vec3>& collisionNormals)
{
	...
	float maxScale = max(max(transformMatrix[0][0], transformMatrix[1][1]), transformMatrix[2][2]);
